...
재귀 함수의 시간 복잡도를 직관적으로 분석하는 것이 어렵다.
분석을 위해 점화식을 세우고, 이를 다양한 방법으로 해석하여 시간 복잡도를 분석한다.
점화식은 더 작은 입력에 대한 자신의 식으로 함수를 나타내는 방정식 또는 부등식이다.

1. 치환법(Substitution Method)
2. 재귀 트리 방법(Recursion-Tree Method)
3. 마스터 방법(Master Method)
4. 아크라 - 바지 방법(Akra-Bazzi Method)

위 네 가지 방법을 상황에 따라 적절히 선택하면, 재귀 함수의 시간 복잡도를 효과적으로 분석할 수 있다.
