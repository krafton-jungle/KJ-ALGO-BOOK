...
> [!NOTE]
> 단일 원소 집합 인경우 자기 자신을 root로 삼는다. 필요시 이와 같이 초기화하여 사용한다.
>
> - 예시
> ```py
...
- **Union(x, y)**  
  ...
  - 예시)
    ...
    def union(x, y):
        ...
        if rootX != rootY:       # 두 노드가 서로 다른 집합에 속한다면
           parent[rootY] = rootX # 부모 노드를 일치시킨다.
    ```
...
- **경로 압축(Path Compression)**  
  ...
  결과적으로 이후 `Find` 호출이 매우 빠르게(거의 O(n)) 수행된다.
